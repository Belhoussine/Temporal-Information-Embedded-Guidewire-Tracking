...
lr_list = [1e-4]
TrainInstances = 12000  # Size of training dataset 1200
ValInstances = 2000    # Size of Validation dataset 2000
BatchSize = 64
ValBatchSize = 16
...
for learning_rate in lr_list:
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(total=n_iter, desc='Epoch: [%d/%d]' % (epoch+1, num_epochs), miniters=1) as t:
            for ii, (L, S, D) in enumerate(train_loader):
                # set the gradients to zero at the beginning of each epoch
                optimizer.zero_grad()
                for ii in range(BatchSize): # ursprünglich: for ii in range(BatchSize):
                    # speed up@zrp
                    if torch.sum(S[ii]) == 0: # ursprünglich: fro torch.sum(S[ii]) == 0:
                        continue
        ...
        starttime = time.time()
        with torch.no_grad():
            for jj, (Lv, Sv, Dv) in enumerate(val_loader):
        ...
        if True or loss_val_mean < minloss:
            ...
            torch.save(net.state_dict(),
                       'Results/%s_%s_Model_al%.2f_Tr%s_epoch%s_lr%.2e_5frames_cropped.pkl' \
